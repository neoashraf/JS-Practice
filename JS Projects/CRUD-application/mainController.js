function saveUser() {
	...
	if (validateTheValues(user)) {
		...
		actionConfirmation.innerHTML = "Please give proper information to complete your registration process."
	}
...
function updateCellValue(cellToEdit, key, col) {

	var user;
	var updatedValue = cellToEdit.innerText;
	//console.log(cellToEdit, key, col);
	var users = JSON.parse(localStorage.getItem('users'));
...
function validateTheValues(user) {
	// console.log(user);
	// if(validate.isString(user.name) && validate.isInteger(user.id) && validate.isDate(user.birthDay) && validate.isNumber(user.cgpa) && validate.isNumber(user.contactNumber))
	// 	return true;
	return true;
}


//onclick="pk(event,\''+ key +'\')"

// var cIndex,rIndex,editEvent;

// function pk(event,key){
